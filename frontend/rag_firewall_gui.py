...
NOTCH_URL = "http://127.0.0.1:8000/verify"
...
class VectorNotchApp:
    def __init__(self, root):
        self.root = root
        self.root.title("VectorNotch - Closed Domain Verification Gateway")
    ...
    def build_ingestion_panel(self):
        ...
        tk.Label(left_frame, text="Data is stored permanently in ./my_local_brain_v3", fg="#576574", bg="#1e272e", font=("Segoe UI", 8)).pack(side=tk.BOTTOM, pady=10)

    def build_chat_panel(self):
        '''Zone 2: Ask Questions & Get the VectorNotch Report'''
        right_frame = tk.Frame(self.root, bg="#f5f6fa", padx=20, pady=20)
        right_frame.grid(row=0, column=1, sticky="nsew")

        tk.Label(right_frame, text="🛡️ Ask LLM (VectorNotch Protected)", fg="#2f3640", bg="#f5f6fa", font=("Segoe UI", 16, "bold")).pack(pady=(0, 15))
    ...
    def process_query(self):
        ...
        try:
            ...
            if "[MISSING_CONTEXT]" in llm_answer:
                ...
                self.append_chat(f"🛡️ VECTORNOTCH BYPASSED (No Data)\n🤖 AI: {clean_answer}", "safe")
                return # Exit before DeBERTa gets confused

            # 4. VectorNotch Validation (Only runs if the AI actually generated a real answer)
            payload = {
                "question": query,          # This must be included!
                "premise": retrieved_context,
                "hypothesis": llm_answer
            }
            response = requests.post(NOTCH_URL, json=payload)
            ...
            if decision.get("is_safe"):
                self.append_chat(f"✅ VECTORNOTCH PASSED (Confidence: {confidence:.1f}%)\n🤖 AI: {llm_answer}", "safe")
            else:
                self.append_chat(f"❌ VECTORNOTCH BLOCKED (Confidence: {confidence:.1f}%)\n⚠️ AI Hallucination Intercepted! The AI tried to say:\n'{llm_answer}'", "blocked")
